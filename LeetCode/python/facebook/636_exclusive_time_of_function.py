...
# 2018-05-08 00:46
# 2018-05-08 01:35 read func id as str, so the final sort wrong..., else finish at 01:12
...
class Solution(object):
    def exclusiveTime(self, n, logs):
        ...
        for l in logs:
            ...
            if len(stack) != 0:
                lfunc, ltag, lt, lint = stack[-1]
                if lfunc == func and ltag == "start" and tag == "end":
                    stack.pop()
                    time = t - lt + 1 - lint

                    if len(stack) != 0:
                        stack[-1][3] = stack[-1][3] + time + lint
                    ...
                    stack.append([func, tag, t, 0])
            else:
                stack.append([func, tag, t, 0])

        return [f[1] for f in sorted(func_time.items(), key=lambda d: d[0])]
