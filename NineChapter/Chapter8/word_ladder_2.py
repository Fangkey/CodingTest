...
class Solution(object):
    def getNeighbors(self, word, word_dict):
        neighbors = []
    ...
    def dfs(self, node, max_layer, node_dict, end_word, list_trace, result):
        if len(list_trace) >= max_layer:
            return

        for n in self.getNeighbors(node, node_dict):
            if node_dict[n] < max_layer:
                if n == end_word:
                    l = list_trace[:]
                    l.append(n)
                    l.reverse()
                ...
                list_trace.append(n)
                self.dfs(n, max_layer, node_dict, end_word, list_trace, result)
                list_trace.pop()
    ...
    def findLadders(self, beginWord, endWord, wordList):
        ...
        node_layer_dict[beginWord] = 1
        layer = 0
        shortest = sys.maxint
        while len(node_queue) != 0 and layer + 1 < shortest:
            layer += 1
            for i in range(0, size):
                cur_node = node_queue.popleft()
                for n in self.getNeighbors(cur_node, word_dict):
                    if n == endWord and not n in node_layer_dict:
                        shortest = layer + 1
                        break
                    else:
                        if not n in node_layer_dict:
                            node_queue.append(n)
                            if layer + 1 < shortest:
                                node_layer_dict[n] = layer + 1
            size = len(node_queue)
            if shortest < sys.maxint:
                break

        result = []
        self.dfs(endWord, shortest, node_layer_dict, beginWord, [endWord], result)
        return result
